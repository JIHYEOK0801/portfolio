...
- ### 개발 주제
  ...
  KOSPI 지수에 영향을 미치는 요인들과 상관관계를 분석하고 머신러닝을 이용해 KOSPI지수 예측
...
2. #### DB 데이터 갱신
   ...
   갱신된 FireBase DB(예측에 필요한 데이터 : dailydata // UI에 쓰이는 데이터 : dailykospi_android)
...
5. #### 스케줄러
   ...
   - 1,2,3,4 실행에 필요한 모듈들을 정해진 시간에 수행하는 파이썬 스케줄러 모듈
   ...
   schedule.every().saturday.at("03:00").do(runmodel) ## 매주 토요일마다 모델 실행
