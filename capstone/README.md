...
- ### 개발 목적
  ...
  캡스톤 디자인 교과목에서 제작한 결과물로, 한 프로젝트의 설계, 구현을 포함하여 모든 개발 과정을 경험해 보기 위해 진행한 프로젝트
...
1. ### Machine Learning
   ...
   - 머신러닝
     ...
     - 예측 과정
       ...
       - 예측을 시행하는 날은 매주 금요일, 목표 예측 날은 5일 후(근무일 기준)로, 매주 한 번씩 실행

       - 5일 후의 KOSPI지수를 예측하기 위해

         1. 예측 시행날 기준 1일 후의 예측 지수 저장
         2. 1 에서 얻은 예측 지수를 기존의 데이터에 합하여 2일 후 예측
         3. 1, 2 의 과정을 5번 반복하여 5일 후의 지수 예측
         4. 예측 시행날 지수와 비교하여 등락 표현
...
3. ### Server / DB
   ...
   Server : KOSPI 지수 예측에 필요한 데이터를 수집하고 데이터를 DB에 저장한다.

   DB : KOSPI 지수 예측에 필요한 실질적인 데이터가 들어있다.
   ...
   - Server
     ...
     1. 모델 예측에 필요한 데이터 수집
        ...
        개발 초기, 모델 예측에 필요한 데이터 컬럼이 확실히 결정되지 않았기 때문에 수집할 수 있는 데이터를 모두 수집하여 DB에 저장한 후 

        추후 모델이 실행될 때 필요한 데이터 컬럼을 선택하여 쓰기로 하였다.
   
        </br>
   
        데이터 수집 방법에는 두가지 방법을 사용하였다.
     ...
     2. 수집한 데이터를 DB에 저장
        ...
        1.에서 '크롤링'과 'API'를 사용하여 수집한 데이터는 날마다 새로 갱신이 되는 데이터이다. 따라서 매일 DB를 갱신하는 코드를 작성하였다.
   
         데이터를 가져오는 시간 또한 다르기 때문에 (각 국가의 장 마감시간의 차이) 이를 해결하기 위해 DB갱신을 1,2차로 나누었다.
   
        </br>
   
        1차 갱신 : API를 사용하여 '환율' 데이터만 갱신
   
        2차 갱신 : DB에서 1차 갱신한 데이터에 2차 갱신 데이터를 추가하여 최종적으로 해당 날의 데이터가 완성되어 DB에 저장
   ...
   - DB
     ...
     1. 모델 예측에 필요한 데이터 저장
        ...
        크롤링과 API를 사용하여 DB에 데이터를 저장하면 다음과 같이 해당날짜에 수집한 데이터가 갱신된다.
     ...
     2. Application의 코스피 차트에 필요한 과거 데이터 저장
        ...
        DB에는 2009-01-12 ~ 현재(2021-06-02) 의 KOSPI 지수 데이터가 들어있다.
        ...
        위의 코드를 실행하면 다음과 같이 오늘의 KOSPI 지수가 DB에 갱신된다.
     ...
     위의 코드를 실행하면 다음과 같이 예측한 KOSPI 지수의 등락 여부가 DB에 저장된다. (1 : 상승, 0 : 하락)
